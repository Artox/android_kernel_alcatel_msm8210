...
#define TAOS_THD_H_OFFSET 115
#define TAOS_THD_L_OFFSET 55
#endif
...
static void taos_get_prox(struct tsl2772_chip *chip)
{
    ...
    //dev_err(&chip->client->dev, "%s: chip->params.prox_th_min %d, chip->params.prox_th_max %d chip->prx_inf.raw %d \n", __func__,
	//		chip->params.prox_th_min, chip->params.prox_th_max,chip->prx_inf.raw);
...
printk(KERN_INFO "taos_%s: ps-distant ---------------------------------- %s ",__func__,chip->prx_inf.detected?"near" : "far" );        
#ifdef PRINT_TIME
...
static irqreturn_t taos_irq(int irq, void *handle)
{
	...
	dev_dbg(dev, "%s\n", __func__);
    disable_irq_nosync(chip->client->irq); //add by junfeng.zhou . 
	(void)taos_check_and_report(chip);
    enable_irq(chip->client->irq); //add by junfeng.zhou . 
bypass:
...
{	
    ...
    //flush_regs(chip);  //add by junfeng.zhou .   
...
static void set_pltf_settings(struct tsl2772_chip *chip)
{
	...
	if (s) {
		...
		sh[TSL277X_CONFIG] = 0;
		sh[TSL277X_PRX_PULSE_COUNT] = chip->pdata->ps_ppcount;
...
static int taos_device_ctl(struct tsl2772_chip *ps_data, bool enable)
{
	...
	if (enable && !ps_data->power_enabled) {
		...
		mdelay(10);
        flush_regs(ps_data);//add by junfeng.zhou . 
...
static int taos_is_sunshine(struct i2c_client *client)
{
	...
	if (ch1_val > (MAX_IR(als_time)*8/10))
	{
		ret = 1;
	}
...
/*	rc = taos_i2c_write(chip, TSL277X_ENABLE, 0x05);
	if(rc)
	{
		printk(KERN_ERR "%s fail, rc=%d", __func__, rc);	
		return rc;
	...
	printk(KERN_INFO "%s:avg_data:%d",__func__,data_cali);
	if(data_cali == -1)
	    goto cali_fail;
	if((data_cali>=0)&&(data_cali<50))
	{   
		databuf[1] = 0X80 | min(420*8/5/chip->pdata->ps_ppcount, 0x7f);
		...
		databuf[1] = 0X80 | min(300*8/5/chip->pdata->ps_ppcount, 0x7f);
		rc = taos_i2c_write(chip, TSL277X_REG_PRX_OFFS, databuf[1]);
    		if(rc)
    		{
    			printk(KERN_ERR "%s fail, rc=%d", __func__, rc);	
		...
		databuf[1] = 0X80 | min(180*8/5/chip->pdata->ps_ppcount, 0x7f);
		rc = taos_i2c_write(chip, TSL277X_REG_PRX_OFFS, databuf[1]);
    		if(rc)
    		{
    			printk(KERN_ERR "%s fail, rc=%d", __func__, rc);	
		...
		databuf[1] = 0X00 | min(300*8/5/chip->pdata->ps_ppcount, 0x7f);
		rc = taos_i2c_write(chip, TSL277X_REG_PRX_OFFS, databuf[1]);
    		if(rc)
    		{
    			printk(KERN_ERR "%s fail, rc=%d", __func__, rc);	
		...
		databuf[1] = 0X00 | min(480*8/5/chip->pdata->ps_ppcount, 0x7f);
		rc = taos_i2c_write(chip, TSL277X_REG_PRX_OFFS, databuf[1]);
    		if(rc)
    		{
    			printk(KERN_ERR "%s fail, rc=%d", __func__, rc);	
...
cali_fail:
    ...
    databuf[1] = 0;
	printk(KERN_INFO "%s: close  = %d,far_away = %d,offset = 0x%X\n",__func__,ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->offset);
...
static int taos_prox_enable(struct tsl2772_chip *chip, int on)
{
	...
	if (on) {
		...
		if (rc)
			return rc;
//add by junfeng.zhou for fix second calling bug begin . 20140670
        {
...
//add by junfeng.zhou for fix second calling bug end .
...
static struct device_attribute prox_attrs[] = {
	...
	__ATTR(write_reg,0664,NULL, taos_write_reg_store),
};
...
bypass_als_idev:
    ...
    chip->client->irq = chip->irq; //add by junfeng.zhou . 
	...
	ret = request_threaded_irq(chip->irq, NULL, taos_irq,
    ...
    disable_irq(chip->irq); //add by junfeng.zhou . 
