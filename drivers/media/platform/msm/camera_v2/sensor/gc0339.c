...
static const struct dev_pm_ops gc0339_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(gc0339_suspend, gc0339_resume)
};
...
static int32_t gc0339_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint16_t chipid = 0;
	uint32_t gpio_ctype = 84;	//Ctype : GPIO84
	
	pr_err("%s: e-welley %d: E\n", __func__, __LINE__);
	...
	ewelley_module = 1;
	pr_err("sub camera sensor e-welley.\n");
	pr_err("%s: %d: X\n", __func__, __LINE__);
