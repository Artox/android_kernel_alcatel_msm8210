...
irqreturn_t msm_dsi_isr_handler(int irq, void *ptr)
{
	...
	if (isr & DSI_INTR_CMD_MDP_DONE)//cwying add for mdp irq handler 2014.7.25
...
#if 0
//cwying add mdp host err  handler 2014.7.25
...
void msm_dsi_cmdlist_commit(struct mdss_dsi_ctrl_pdata *ctrl, int from_mdp)
{
	...
	//dsi_set_tx_power_mode(0); //cwying deleted for nt35517 write code failed 2014.7.25
...
#if 0
//cwying add read LCD ID 2014.7.25
static char panel_id_cmd[] = {0xBE, 0x00, 0x06, 0xA0};

static struct dsi_cmd_desc dsi_cmd = {
	{DTYPE_DCS_READ, 1, 0, 1, 5, sizeof(panel_id_cmd)},
	panel_id_cmd};

static char cmd0[] = {0xF0,0x55,0xAA,0x52,0x08,0x01};

static struct dsi_cmd_desc dsi_cmd_page = {
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(cmd0)}, 
	cmd0};
	
void msm_dsi_read_status(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	memset(rp->data, 0, 4);
	dsi_set_tx_power_mode(0);
	msm_dsi_cmds_rx(ctrl_pdata, &dsi_cmd_page, 1);
	msm_dsi_cmds_rx(ctrl_pdata, &dsi_cmd, 1);
	dsi_set_tx_power_mode(1);
	panel_id = rp->data[0];
	pr_err("Panel 0x0a ID =0x%x, count = %d", panel_id, count++);
